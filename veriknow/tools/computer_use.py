...
class ComputerUseVerifier:
    def __init__(self, safety: ComputerUseSafetyConfig | None = None):
    ...
    def verify_step(
        ...
        *,
        ...
        observations = [f"instruction={instruction}", f"url={url}"]
        ...
        observations.append("computer-use runtime is not configured; traceable boundary recorded")
        reason = (
        ...
        self._write_log(log_path, url, expected_result, actions, observations, "partial", reason)
        return ComputerUseObservation(
            status="partial",
            actual_result=reason,
